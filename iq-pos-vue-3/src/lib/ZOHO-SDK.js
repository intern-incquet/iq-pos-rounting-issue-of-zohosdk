...
var ZSDKUtil = function (c) {
    function f() { } function g(b) { var a = {}; b = b || window.location.href; return b.substr(b.indexOf("?") + 1).split("\x26").forEach(function (b) { b = b.split("\x3d"); a[b[0]] = b[1] }), a.hasOwnProperty("serviceOrigin") && (a.serviceOrigin = decodeURIComponent(a.serviceOrigin)), a } var e, b = g(); return f.prototype.Info = function () { (c.isDevMode() || c.isLogEnabled()) && console.info.apply(console, arguments) }, f.prototype.Error = console.error, c.GetQueryParams = g, c.isDevMode = function () { return b && b.isDevMode }, c.isLogEnabled =
        function () { return b && b.isLogEnabled }, c.getLogger = function () { return e && e instanceof f || (e = new f), e }, c
}(window.ZSDKUtil || {}), ZSDKMessageManager = function (c) {
    function f(a) {
        try { var e = "string" == typeof a.data ? JSON.parse(a.data) : a.data } catch (u) { e = a.data } var c = e.type; try {
            var f; if (!(f = "__REGISTER__" === c)) { var l = a.source, k = a.origin; f = !(!h.isRegistered() || p !== l || v !== k) || Error("Un-Authorized Message.") } if (f) switch (c) {
                case "__REGISTER__": a = e; p = window.parent; v = w.serviceOrigin; h.dcType = window.location.origin.split(".").pop().toUpperCase();
                    "COM" === h.dcType && (h.dcType = "US"); h.key = a.uniqueID; h.parentWindow = p; h._isRegistered = !0; d({ type: "__REGISTER__", widgetOrigin: q(), uniqueID: h.key }, h); b(h, "Load", a.data); break; case "__EVENT_RESPONSE__": var n = e.promiseID, r = e.data, t = e.isSuccess; m.hasOwnProperty(n) && (t ? m[n].resolve(r) : m[n].reject(r), m[n] = void 0, delete m[n]); break; default: g(a, e)
            }
        } catch (u) { x.Error("[SDK.MessageHandler] \x3d\x3e ", u.stack) }
    } function g(a, d) {
        var c = d.widgetID; a = d.eventName; if (h.key === c) var f = b(h, a, d.data); else {
            var l = h._childWidgets[c];
            l && (f = b(l, a, d.data))
        } if (d.isPromise) { var k = {}; Promise.all(f).then(function (a) { k.response = a; k.widgetID = c; k.sourceWidgetID = h.key; e(d, k) }).catch(function (a) { k.response = a; k.widgetID = c; k.sourceWidgetID = h.key; e(d, k) }) }
    } function e(a, b) { d({ type: "__EVENT_RESPONSE__", widgetOrigin: q(), uniqueID: h.key, eventName: a.eventName, data: b, promiseID: a.promiseID }, h) } function b(a, b, d) {
        b = a.eventHandlers[b]; var e = []; if (Array.isArray(b)) for (var c = 0; c < b.length; c++) {
            try {
                var f = b[c].call(a, d); var l = f instanceof Promise ? f.then(function (a) {
                    return {
                        isSuccess: !0,
                        response: a
                    }
                }).catch(function (a) { return { isSuccess: !1, response: a } }) : { isSuccess: !0, response: f }
            } catch (r) { l = { isSuccess: !1, response: r } } e.push(l)
        } return e
    } function d(b, d) { var e, c = b.isPromise; if (c && (e = "Promise" + t++, b.promiseID = e), d && (b.uniqueID = (d.parentWidget || d).key, b.widgetID = d.key), b.time = (new Date).getTime(), k(b), c) return a(e) } function a(a) { return new Promise(function (b, d) { m[a] = { resolve: b, reject: d, time: (new Date).getTime() } }) } function l() { k({ type: "__DEREGISTER__", uniqueID: h.key }) } function k(a) {
        if ("object" ==
            typeof a && (a.widgetOrigin = encodeURIComponent(q())), !p) throw Error("Parentwindow reference not found."); p.postMessage(a, w.serviceOrigin)
    } function q() { return window.location.protocol + "//" + window.location.host + window.location.pathname } var h, p, v, x = ZSDKUtil.getLogger(), t = 100, m = {}, w = ZSDKUtil.GetQueryParams(); return c.Init = function (a) { h = a; window.addEventListener("message", f); window.addEventListener("unload", l) }, c.SendEvent = d, c
}(window.ZSDKMessageManager || {});
window.ZSDK = function () {
    function c(e) { this.serviceOrigin = e.serviceOrigin || g.serviceOrigin; this.parentWidget = e.parentWidget; this.key = e.key; this._isRegistered = !1; this._childWidgets = {}; this.eventHandlers = Object.create(null); this.meta } var f, g = ZSDKUtil.GetQueryParams(); return c.prototype.on = function (e, b) {
        if ("string" != typeof e) throw Error("Invalid eventname parameter passed."); if ("function" != typeof b) throw Error("Invalid function parameter passed."); var d = this.eventHandlers[e]; if (Array.isArray(d) || (this.eventHandlers[e] =
            d = []), d.push(b), "Load" !== e) { var a = { type: "__EVENT_BIND__", eventName: e, count: d.length }; this.parentWidget && !this.parentWidget.isRegistered() || !this.parentWidget && !this.isRegistered() ? (this.parentWidget || this).on("Load", function () { ZSDKMessageManager.SendEvent(a, this) }) : ZSDKMessageManager.SendEvent(a, this) }
    }, c.prototype.off = function (e, b) {
        if ("string" != typeof e) throw Error("Invalid eventname parameter passed."); if ("function" != typeof b) throw Error("Invalid function parameter passed."); e = this.eventHandlers[e];
        if (Array.isArray(e)) { var d, a = []; for (d = 0; d < e.length; d++)e[d] === b && a.push(d); for (; a.length;)e.splice(a.pop(), 1) }
    }, c.prototype._sendEvent = function (e, b, d) { return ZSDKMessageManager.SendEvent({ type: "__EVENT__", eventName: e, data: b, isPromise: d }, this) }, c.prototype.emit = function (e, b) { ZSDKMessageManager.SendEvent({ type: "__EMIT__", eventName: e, data: b }, this) }, c.prototype.isRegistered = function () { return this._isRegistered }, c.prototype.fetch = function (e) {
        ...
        var b = this._childWidgets[e]; return b || (this._childWidgets[e] = b = new c({ key: e, parentWidget: this })), b
...
window.SigmaSDK = function () {
    function c(a) { this._serviceName = a } var f, g = function () { if (!f.isRegistered()) throw Error("App not registered."); }, e = function (a, b) { return f._sendEvent("SIGMA_" + a + "_EVENT", b, !0) }, b = function (a, b) { return Promise.reject({ type: a, message: b }) }; c.prototype.isRegistered = function () { return f.isRegistered() }; c.prototype.context = function () { return g(), f }; c.prototype.createWidget = function (a) { return g(), f.createInstance(a) }; c.prototype.getWidget = function (a) { return f.getWidgetInstance(a) }; c.prototype.widgetsMeta =
        function () { return g(), f.getWidgets() }; c.prototype.on = function (a, b) { return f.on(a, b) }; c.prototype.off = function (a, b) { return f.off(a, b) }; c.prototype.trigger = function (a, b) { return g(), f.emit(a, b) }; c.prototype.modal = function (a) { return g(), f.modal(a) }; c.prototype.fetch = function (a) { return g(), f.fetch(a) }; c.prototype.get = function (a, d) {
            if (g(), "string" != typeof a && !Array.isArray(a)) return b("VALIDATION_ERROR", "The get method accepts String or Array of Strings only."); a = "string" == typeof a ? [a] : a; if (0 >= a.length) return b("VALIDATION_ERROR",
                "The get method should not accept empty Array of Strings."); a = { event_type: "get", properties: a }; return "function" == typeof d && f.on("get", d), e(this._serviceName, a)
        }; c.prototype.getAll = function (a, d, c) { if (g(), "string" != typeof a) return b("VALIDATION_ERROR", "The getAll method accepts module_name as String only."); a = { event_type: "get_all", module_name: a, options: d }; return "function" == typeof c && f.on("get_all", c), e(this._serviceName, a) }; c.prototype.set = function (a, d, c) {
            if (g(), "string" != typeof a && "object" != typeof a &&
                Array.isArray(a)) return b("VALIDATION_ERROR", "The set method accepts key-value pair or Object of key-value pairs only."); var k; if ("string" == typeof a ? (k = {}, k[a] = d) : k = a, 0 >= k.length) return b("VALIDATION_ERROR", "The set method should not accept empty Object."); a = { event_type: "set", properties: k }; return "function" == typeof c && f.on("set", c), e(this._serviceName, a)
        }; c.prototype.remove = function (a, d) {
            if (g(), "string" != typeof a && !Array.isArray(a)) return b("VALIDATION_ERROR", "The remove method accepts String or Array of Strings only.");
            a = "string" == typeof a ? [a] : a; if (0 >= a.length) return b("VALIDATION_ERROR", "The remove method should not accept empty Array of Strings."); a = { event_type: "remove", properties: a }; return "function" == typeof d && f.on("remove", d), e(this._serviceName, a)
        }; c.prototype.request = function (a, d) {
            if (g(), !a.url || 0 >= a.url.trim().length) return b("VALIDATION_ERROR", "The parameter url should not be empty"); if ("string" != typeof a.url) return b("VALIDATION_ERROR", "The parameter url should be type of string"); if (a.method || (a.method =
                "GET"), a.params && "object" != typeof a.params && Array.isArray(a.params)) return b("VALIDATION_ERROR", "The parameter params should be type of object"); if (a.headers && "object" != typeof a.headers && Array.isArray(a.headers)) return b("VALIDATION_ERROR", "The parameter headers should be type of object"); if (a.files) if (Array.isArray(a.files)) for (var c = 0; c < a.files.length; c++)a.files[c] = f.getFileObject(a.files[c]); else {
                    if ("object" != typeof a.files) return b("VALIDATION_ERROR", "The parameter files should be type of object");
                    if (5 < Object.keys(a.files).length) return b("VALIDATION_ERROR", "You can upload a maximum of 5 files at a time.")
                } a = { event_type: "request", options: a }; return "function" == typeof d && f.on("request", d), e(this._serviceName, a)
        }; c.prototype.requestapiconnection = function (a, d) {
            if (g(), !a.api_namespace || 0 >= a.api_namespace.trim().length) return b("VALIDATION_ERROR", "The parameter api_namespace should not be empty"); if ("string" != typeof a.api_namespace) return b("VALIDATION_ERROR", "The parameter api_namespace should be type of string");
            if (!a.url || 0 >= a.url.trim().length) return b("VALIDATION_ERROR", "The parameter url should not be empty"); if ("string" != typeof a.url) return b("VALIDATION_ERROR", "The parameter url should be type of string"); if (a.method || (a.method = "GET"), a.params && "object" != typeof a.params && Array.isArray(a.params)) return b("VALIDATION_ERROR", "The parameter params should be type of object"); if (a.headers && "object" != typeof a.headers && Array.isArray(a.headers)) return b("VALIDATION_ERROR", "The parameter headers should be type of object");
            if (a.files) if (Array.isArray(a.files)) for (var c = 0; c < a.files.length; c++)a.files[c] = f.getFileObject(a.files[c]); else { if ("object" != typeof a.files) return b("VALIDATION_ERROR", "The parameter files should be type of object"); if (5 < Object.keys(a.files).length) return b("VALIDATION_ERROR", "You can upload a maximum of 5 files at a time.") } a = { event_type: "requestapiconnection", options: a }; return "function" == typeof d && f.on("requestapiconnection", d), e(this._serviceName, a)
        }; c.prototype.dispatch = function (a, b, d) {
            g();
            b = { event_type: a, options: b }; return "function" == typeof d && f.on(a, d), e(this._serviceName, b)
        }; var d = function (a) { return f = window.ZSDK.Init(), "function" == typeof a && (f.isRegistered() ? a.call() : f.on("Load", a)), new c(this._serviceName) }; return {
            CRM: { init: d.bind({ _serviceName: "CRM" }) }, DESK: { init: d.bind({ _serviceName: "DESK" }) }, PROJECTS: { init: d.bind({ _serviceName: "PROJECTS" }) }, ORCHESTLY: { init: d.bind({ _serviceName: "ORCHESTLY" }) }, MAIL: { init: d.bind({ _serviceName: "MAIL" }) }, SHOW: { init: d.bind({ _serviceName: "SHOW" }) },
            SDP: { init: d.bind({ _serviceName: "SDP" }) }, IOT: { init: d.bind({ _serviceName: "IOT" }) }, ZAPPS: { init: d.bind({ _serviceName: "ZAPPS" }) }, CATALYST: { init: d.bind({ _serviceName: "CATALYST" }) }, FINANCE: { init: d.bind({ _serviceName: "FINANCE" }) }, CONNECT: { init: d.bind({ _serviceName: "CONNECT" }) }, TEAMINBOX: { init: d.bind({ _serviceName: "TEAMINBOX" }) }, SPRINTS: { init: d.bind({ _serviceName: "SPRINTS" }) }, BUGTRACKER: { init: d.bind({ _serviceName: "BUGTRACKER" }) }, CREATOR: { init: d.bind({ _serviceName: "CREATOR" }) }, PEOPLE: { init: d.bind({ _serviceName: "PEOPLE" }) },
            COMMERCE: { init: d.bind({ _serviceName: "COMMERCE" }) }, SITES: { init: d.bind({ _serviceName: "SITES" }) }, RECRUIT: { init: d.bind({ _serviceName: "RECRUIT" }) }, WORKDRIVE: { init: d.bind({ _serviceName: "WORKDRIVE" }) }, WRITER: { init: d.bind({ _serviceName: "WRITER" }) }, INVOICE: { init: d.bind({ _serviceName: "INVOICE" }) }, INVENTORY: { init: d.bind({ _serviceName: "INVENTORY" }) }, SUBSCRIPTIONS: { init: d.bind({ _serviceName: "SUBSCRIPTIONS" }) }, CAMPAIGNS: { init: d.bind({ _serviceName: "CAMPAIGNS" }) }, CHARMHEALTHEHR: { init: d.bind({ _serviceName: "CHARMHEALTHEHR" }) },
            BIGIN: { init: d.bind({ _serviceName: "BIGIN" }) }, EXPENSE: { init: d.bind({ _serviceName: "EXPENSE" }) }, FSM: { init: d.bind({ _serviceName: "FSM" }) }
        }
}(); var ZCSDK = new function () {
...
function Records(c) {
    function f(b) { b.scope || (b.scope = c.initParams.scope); b.envUrlFragment || (b.envUrlFragment = c.initParams.envUrlFragment.substr(1)); b.appName || (b.appName = c.initParams.appLinkName) } function g(b, d) { var a = !1, c; for (c in b) { var e; if (e = !(d && d.includes(c))) { e = b[c]; var f = !1, g; if (!(g = !e || null == e || "" === e || "string" == typeof e && 0 == e.trim().length) && (g = "object" == typeof e)) a: { g = void 0; for (g in e) { g = !1; break a } g = !0 } g && (f = !0); e = f } e && (a = !0) } return a } function e(b) {
        return new Promise(function (d, a) {
            var c =
                new FileReader; c.readAsDataURL(b); c.onload = function () { d(c.result) }; c.onerror = function (b) { a(b) }
        })
    } return {
        addRecord: function (b) { if (g(b)) return new Promise(function (b, a) { a("Improper Configuration..!!") }); f(b); return c.appSDK._sendEvent("ADD_RECORD", { appLinkName: b.appName, formLinkName: b.formName, body: b.data }, !0) }, updateRecord: function (b) {
            if (g(b)) return new Promise(function (a, b) { b("Improper Configuration..!!") }); var d = b.id.toString().split(","); f(b); return c.appSDK._sendEvent("EDIT_RECORDS", {
                appLinkName: b.appName,
                viewLinkName: b.reportName, body: b.data, listOfRecords: d
            }, !0)
            ...
            if (g(b, ["file", "parentId"])) return new Promise(function (b, a) { a("Improper Configuration..!!") }); f(b); return b.file ? b.file.size && 50 < b.file.size / 1024 / 1024 ? new Promise(function (b, a) { a("Improper Configuration..!!") }) : new Promise(function (d,
                a) { var f = e(b.file), g = "", q = b.file.name; f.then(function (e) { g = e; e = { appLinkName: b.appName, viewLinkName: b.reportName, id: b.id, fieldName: b.fieldName, file: g, fileName: q }; b.parentId && (e.parentId = b.parentId); c.appSDK._sendEvent("UPLOAD_FILE", e, !0).then(function (a) { d(a) }).catch(function (b) { a(b) }) }).catch(function (b) { a(b) }) }) : new Promise(function (b, a) { a("Improper Configuration..!!") })
        }, getRecordCount: function (b) {
            if (g(b)) return new Promise(function (b, a) { a("Improper Configuration..!!") }); f(b); return c.appSDK._sendEvent("GET_COUNT",
                { appLinkName: b.appName, viewLinkName: b.reportName, criteria: b.criteria }, !0)
        }, readFile: function (b) { if (g(b)) return new Promise(function (a, b) { b("Improper Configuration..!!") }); f(b); var d = { appLinkName: b.appName, viewLinkName: b.reportName, id: b.id, fieldName: b.fieldName }; b.parentId && (d.parentId = b.parentId); return c.appSDK._sendEvent("READ_FILE", d, !0) }
    }
}
function Util(c) {
    return {
        setImageData: function (f, g, e) { if (g.startsWith("/api/v2/")) { var b = {}; b.src = g; c.appSDK._sendEvent("IMAGE_LOAD", b, !0).then(function (b) { f.setAttribute("src", b); e({ status: "200", statusText: "success" }) }).catch(function (b) { if (e) e(b); else { var a = window.console; a.log("Error: Unable to set image data"); a.log(b) } }) } else f.setAttribute("src", g) }, getInitParams: function () { return c.initParams }, getQueryParams: function () { return c.queryParams }, navigateParentURL: function (f) {
